...
public class UnbindSkullCommand extends ClientCommand {
	...
	public static final NBTReferenceFilter SKULL_FILTER = NBTReferenceFilter.create(
			...
			null,
			Component.translatableEscape("nbteditor.no_ref.skull"),
			Component.translatableEscape("nbteditor.no_hand.no_item.skull"));
			...
			NBTReference.getReference(SKULL_FILTER, false, ref -> {
				...
				if (ref instanceof ItemReference itemRef) {
					...
					if (!item.nbte$hasCustomName()) {
						item.nbte$setCustomName(Component.translatableEscape("block.minecraft.player_head.named", profile.get().name())
								.withStyle(style -> style.withItalic(false).withColor(ChatFormatting.YELLOW)));
					}
					itemRef.saveItem(item, Component.translatableEscape("nbteditor.unbind_skull.unbound"));
				} else {
					...
					blockRef.saveLocalNBT(block, Component.translatableEscape("nbteditor.unbind_skull.unbound"));
				}
			});
