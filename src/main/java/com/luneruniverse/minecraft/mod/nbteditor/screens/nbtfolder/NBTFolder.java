...
public interface NBTFolder<T extends Tag> {
	...
	public static final Map<Class<? extends Tag>, Constructor<?>> TYPES = getTypesMap();
	private static Map<Class<? extends Tag>, Constructor<?>> getTypesMap() {
		Map<Class<? extends Tag>, Constructor<?>> output = new HashMap<>();
	...
	/** A scan, not a lookup: a folder type registers a supertype, as {@code CollectionTag} does. */
	private static Constructor<?> constructorFor(Class<? extends Tag> nbt) {
		for (Map.Entry<Class<? extends Tag>, Constructor<?>> entry : TYPES.entrySet()) {
			if (entry.getKey().isAssignableFrom(nbt))
	...
	public static <T extends Tag> NBTFolder<T> get(Class<T> nbt, Supplier<T> get, Consumer<T> set) {
		Constructor<?> constructor = constructorFor(nbt);
		if (constructor == null)
			return null;
