...
public class MultiLineTextFieldWidget implements Renderable, MVElement, Tickable, NarratableEntry {
	...
	public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
		...
		for (Component line : renderedLines) {
			context.text(textRenderer, line, x + textRenderer.lineHeight, yOffset + textRenderer.lineHeight, -1, shadow);
		...
		if (isMultiFocused() && cursorBlinkTracker / 6 % 2 == 0) {
			...
			context.fill(cursor.x, cursor.y, cursor.x + 1, cursor.y + textRenderer.lineHeight, cursorColor);
		}
	...
	protected void renderHighlight(GuiGraphicsExtractor context, int start, int end, int color) {
		...
		if (startPos.y == endPos.y)
			context.fill(startPos.x, startPos.y, endPos.x, endPos.y + textRenderer.lineHeight, color);
		else {
			int line = 0;
			int lineY;
			while ((lineY = startPos.y + line * (int) (textRenderer.lineHeight * 1.5)) < endPos.y) {
				...
				context.fill(lineStart.x, lineStart.y, x + width - textRenderer.lineHeight, lineStart.y + textRenderer.lineHeight, color);
				line++;
			}
			context.fill(x + textRenderer.lineHeight, lineY, endPos.x, endPos.y + textRenderer.lineHeight, color);
		}
	}
