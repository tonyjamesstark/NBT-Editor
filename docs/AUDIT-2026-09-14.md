...
- [x] 13. Fold `ClassMap` into `NBTFolder`; narrow `OrderedMap`'s surface; drop `Reflection`'s cache
      Three commits, `49d7685..b7f98f6`, each verified to build on its own. Net 269 lines gone
      and two `util/` types deleted.
      `ClassMap`: the scan for a registered key assignable from a tag's class is a folder-type
      concern, not a general one, so `TYPES` is a plain `HashMap` and the scan is a private method
      beside it. First-assignable-wins in hash order is preserved exactly, so an addon registering
      overlapping types gets the answer it got before. `NBTEditorAPI.registerNBTFolderType`
      compiles untouched.
      `OrderedMap`: deleted outright rather than narrowed. `ConfigGrouping.paths` is a
      `LinkedHashMap` and `sort` is four lines, sort the keys and move each to the end. The keys
      are only ever `Integer` (`ConfigList`) or `String` (`ConfigCategory`, `ConfigBar`), so the
      248 lines of linear `equals` scans bought nothing over a hash lookup, and
      `getConfigurables()` still hands out `Collections.unmodifiableMap(paths)`.
      The audit's claim that "the dual insertion-order/sorted mode is genuinely used
      (ConfigList.java:332)" does not survive reading it. `setSorter` set the field and then
      called `sort`, which set the field back to null before sorting, so no standing sorter ever
      survived the call and `put` never re-sorted. Nothing reads `getSorter`. The method is a
      one-off sort and is now written as one, with the javadoc saying so.
      `Reflection`: a Guava `Cache` in front of one lookup made once at startup and stashed in a
      holder field. `Class.forName` already goes through the classloader's cache. Gone, with the
      two checked exception types it forced callers to unwrap; `mappings` is private now.
      **Not done:** `Reflection.FieldReference`, a 20-line wrapper for one call site that only
      uses `get`. Collapsing `getField(...).get(obj)` into one call is a further shrink and was
      not this item. The new `sort` has no test: `ConfigGrouping` imports
      `net.minecraft.client.input`, so it is outside the JVM-only slice per
      `docs/adr/0004-jvm-only-tests.md`. Verified by compile only.
- [x] 14. Clean accumulated per-commit jars from `build/libs`
      44 jars, 56M, down to the one matching `HEAD`. 1.3M.
      **Not fixed at the root:** `scripts/publish-release.sh` builds its jar path from the same
      `${BASE_NAME}-${VERSION}-${HASH}` pattern, so dropping `archiveClassifier` would break
      releasing. The accumulation is a periodic sweep, not a one-line build change.
